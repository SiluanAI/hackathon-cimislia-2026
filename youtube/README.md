# YouTube — Liron Agency (nișa HVAC)

Pachetele complete de video și istoricul strategiei. Skill-ul care le produce:
`.claude/skills/youtube-video-liron/`. Digest pentru sesiuni noi:
`decisions/youtube-strategy.md`.

## Pachete (`pachete/`)

| Fișier | Idee | Subiect | Status |
|---|---|---|---|
| 01-cati-clienti-pierzi.md | #1 | Câți clienți pierzi fără sistem (calcul) | Filmat |
| 02-programare-automata.md | #15 | Programare automată fără telefon | Programat 25 iun |
| 03-site-3-zile.md | #21 | Site Ecoconfortsistem în 3 zile | Programat 26 iun |
| 04-chatbot.md | #11 | Ce e un chatbot, prinde clienții noaptea | Programat 28 iun |
| 05-nu-am-nevoie-de-site.md | #4 | "Nu am nevoie de site" — calcul ~1M lei/an | Programat 30 iun |
| 06-obiectii-programare-online.md | #18 | 3 frici despre programarea online | De filmat |
| 07-google-maps.md | #6 | Instalator mai slab apare pe Google | De filmat |
| 08-sezon-ac.md | #47 | Sezon AC: pregătește-te acum | De filmat |
| 09-recenzii-google.md | nou | Zeci de recenzii fără să ceri manual | De filmat |

## Evoluția strategiei
`evolutie.md` — cum s-a schimbat abordarea de la selecția nișei până acum.

## Reguli rapide
- Format: Explainer. Metrica: LEAD, nu vederi.
- Trifecta: titlu + thumbnail + intro congruente.
- Thumbnail: ChatGPT din script, max 3-4 cuvinte text.
- Descriere: link `liron.md`, capitole ca repere fără timecode.
- Poziționare: Expert-Enthusiast ("cum am făcut eu pentru un client HVAC").
